...
export const RegistrationScreen = observer(() => {
  ...
  const handleLoginChange = (value: string) => {
    setLogin(value);
    setIsLoginValid(value.trim().length >= 3);
  };
  ...
  const handleSendCode = async () => {
    if (!isLoginValid) {
      Alert.alert('Ошибка', 'Введите корректный логин (минимум 3 символа)');
      return;
    }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            ...
            style={[
              styles.textInput,
              !isLoginValid && login.length > 0 && styles.textInputError,
              isLoginValid && styles.textInputValid
            ]}
            autoCapitalize="none"
            autoCorrect={false}
        ...
        <TouchableOpacity
          style={[
            styles.sendButton,
            (!isValid || !isLoginValid || authModel.isLoading) && styles.sendButtonDisabled
          ]}
          onPress={handleSendCode}
          disabled={!isContactValid || !isLoginValid || authModel.isLoading}
        >
          {authModel.isLoading ? (
